...
trait Solver extends GameDef:
  ...
  def from(initial: LazyList[(Block, List[Move])], explored: Set[Block]): LazyList[(Block, List[Move])] =
    if (initial.isEmpty) LazyList.empty
      ...
      val expl = explored ++ (newNeighborsOnly(neighbors, explored) map(_._1))
      val initls = initial.tail ++ newNeighborsOnly(neighbors, explored)
      initial.head #:: from(initls, expl) // return lazylist with initial val
    }
   ...
   */
  lazy val pathsToGoal: LazyList[(Block, List[Move])] = pathsFromStart.filter(a => done(a._1)).reverse
  /**
   ...
   * position.
   */
  lazy val solution: List[Move] = pathsToGoal.map(v => v._2).head.reverse