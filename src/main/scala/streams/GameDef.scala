...
trait GameDef:
  ...
  case class Block(b1: Pos, b2: Pos):
     ...
     */
    def neighbors: List[(Block, Move)] = (left,Move.Left)::(right,Move.Right)::(up,Move.Up)::(down,Move.Down)::Nil

    /**
     * Returns the list of positions reachable from the current block
     * which are inside the terrain.
     */
    def legalNeighbors: List[(Block, Move)] = neighbors.filter((a,b) => a.isLegal)

    /**
     * Returns `true` if the block is standing.
     */
    def isStanding: Boolean = b1 == b2 //when two blocks same loc.

    /**
     * Returns `true` if the block is entirely inside the terrain.
     */
    def isLegal: Boolean = terrain(b1) && terrain(b2) // when two blocks is in terrain
