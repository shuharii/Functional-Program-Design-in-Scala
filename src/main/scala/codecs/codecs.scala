...
trait EncoderInstances:
  ...
  given Encoder[Boolean] =
    Encoder.fromFunction(n => Json.Bool(n))// TODO Define a given value of type `Encoder[Boolean]`
...
trait DecoderInstances:
  ...
  given Decoder[Int] = Decoder.fromFunction {
    case Json.Num(x) =>
      if (x.isValidInt){
        Some(x.toInt)
      }else{
  ...
  given [A] (using decoder: Decoder[A]): Decoder[List[A]] = 
    Decoder.fromFunction {
      case Json.Arr(list) => Option(list.flatMap(decoder.decode))
      //Decoder.fromFunction{
      // as => Json.Arr(as.map(encoder.encode))
      // }
    }
  /**
  ...
  def field[A](name: String)(using decoder: Decoder[A]): Decoder[A] =
    case Json.Obj(a) => a(name).decodeAs
    //Decoder.fromFunction{
    // a => Json.Obj(Map(name -> encoder.encode(a)))
    // }
...
trait PersonCodecs:
  ...
  Decoder.field[String]("name")
    ...
    .transform[Person](a => Person(a._1, a._2.toInt)) // person object
...
trait ContactsCodecs:
  ...
  given Decoder[Contacts] =
    Decoder.field[List[Person]]("people")
      .transform[Contacts]((users: List[Person]) => Contacts(users))
