...
object Calculator extends CalculatorInterface:
 import Expr.*
  ...
  def computeValues(
    ...
    for {
      (va, ex) <- namedExpressions
    } yield { (va -> Signal(eval( ex(), namedExpressions))) }
  def eval(expr: Expr, references: Map[String, Signal[Expr]])(using Signal.Caller): Double =
    expr match {
      ...
      case Divide(a,b) => if (eval(b,references).equals(0)) Double.NaN else eval(a,references) / eval(b,references)
    }
